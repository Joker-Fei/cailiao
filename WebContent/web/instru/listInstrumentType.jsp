...
<body>
 <div class="position">　<a style="font-size:14px;text-decoration:none;color:#FFAA33;" >当前位置：仪器管理　&gt;&gt;　类型管理　</a>

<div class="div1">
<form action="instrumentTypeServlet?op=add" method="post" name="form1" onsubmit="return formCheck33();">
	...
	<tbody>
            ...
            <a href="instrumentTypeServlet?op=list">刷新列表</a>
            </div>
		</table>
	</tbody>
...
<div class="div2">
<form name="myform" method="post" action="instrumentTypeServlet?op=batchDelete">
	<tbody>
	<table class="right-table" width="50%"  border="0px" cellpadding="0" align="center" cellspacing="0">
		...
		<c:forEach items="${requestScope.instrumentTypeList }"  var="instrumentType" varStatus="status">
		  	<tr style="background-color: ${status.index%2==0?'':'#4F4F4F' } ">
				<td>
					<%-- <input type="checkbox" name="ids" value="${instrumentType.id }"/> --%>
					${instrumentType.id }
				</td>
				<td>${instrumentType.typeName }</td>	
				<td><a href="instrumentTypeServlet?op=delete&id=${instrumentType.id }">删除</a></td>
			</tr>
		</c:forEach>
	</table>
	</tbody>
