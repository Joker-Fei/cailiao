...
/**
 * Servlet implementation class PhotoTypeServlet
 */
@WebServlet("/PhotoTypeServlet")
public class PhotoTypeServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;

	/**
	 * @see HttpServlet#HttpServlet()
	 */
	public PhotoTypeServlet() {
		super();
	...
	protected void doPost(HttpServletRequest request,
		...
		if ("add".equals(op)) {
			addPhotoType(request, response);
		} else if ("list".equals(op)) {
			listPhotoType(request, response);
	...
	private void listPhotoType(HttpServletRequest request,
			HttpServletResponse response) throws ServletException, IOException {
		PhotoTypeService photoTypeService = new PhotoTypeService();
		List<PhotoType> photoTypeList = photoTypeService.findAll();
		request.setAttribute("photoTypeList", photoTypeList);
		request.getRequestDispatcher("listPhotoType.jsp").forward(request,
				response);

	}

	private void addPhotoType(HttpServletRequest request,
			HttpServletResponse response) throws IOException {
		String typeName = request.getParameter("typeName");
		PhotoType photoType = new PhotoType(typeName);
		PhotoTypeService photoTypeService = new PhotoTypeService();
		photoTypeService.save(photoType);
		response.sendRedirect("listPhotoType.jsp");// response.sendRedirect的功能是地址重定向(页面跳转).
