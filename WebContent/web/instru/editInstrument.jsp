...
<div class="position">　<a style="font-size:14px;text-decoration:none;color:#FFAA33;" >当前位置：仪器一览　&gt;&gt;　信息编辑　</a>
<div class="div3">
<div class="tableMain">
<div class="div1"> 
<form action="instrumentServlet?op=update" method="post" name="form1" onsubmit="return formCheck33();" id="form1">
	
	<table class="right-table" width="70%" height="90%" border="0px" cellpadding="0" align="center" cellspacing="0" >
		<tr>
			<td><a>仪器类型</a></td>
			<td>
				<input type="hidden" name="id" value="${requestScope.instrument.id }"/>
				<select name="typeId">				
		...
		<tr >
			...
			<td><textarea   rows="1" cols="30" name="factory">${requestScope.instrument.factory }</textarea></td>
			<td><a>主要附件</a></td>
			<td><textarea   rows="1" cols="30" name="mainAccessories" >${requestScope.instrument.mainAccessories }</textarea></td>
		</tr>
		<tr >
			<td><a>技术参数</a></td>					 
			<td><textarea   rows="2" cols="30" name="tecParame" >${requestScope.instrument.tecParame }</textarea></td>
		
			<td><a>技术特点</a></td>
			<td><textarea   rows="2" cols="30" name="tecFeatures" >${requestScope.instrument.tecFeatures }</textarea></td>
		</tr>
		<tr >
			<td><a>应用范围</a></td>
			<td><textarea   rows="3" cols="30" name="appliRange" >${requestScope.instrument.appliRange }</textarea></td>
			
		</tr>
		<tr>
			<td><a>发布时间</a></td>
			<td><input type="text" name="publishTime" onClick="WdatePicker()" class="Wdate" value="${requestScope.instrument.publishTime }"/></td>
		</tr>
		<%-- <tr>
			<td>状态</td>
			<c:if test="${requestScope.instrument.pass eq '1'}">
			<td>
				<input type="hidden" name="pass" value="1" />未通过
			</td>
			</c:if>	
			<c:if test="${requestScope.instrument.pass eq '0'}">	
			<td>
		...
		<tr>
			...
			<td colspan="4" align="center">
				<textarea   rows="8" cols="15" name="content">${requestScope.instrument.content }</textarea>
				<script type="text/javascript">
			    	var editor =CKEDITOR.replace('content', {
						uiColor: '#14B8C4'
					});
		...
		<tr>
			<td colspan="2">
				<input type="submit" value="修改信息" /><!-- onclick="display_alert()" -->
			</td>
		</tr>
	</table>
</form>
