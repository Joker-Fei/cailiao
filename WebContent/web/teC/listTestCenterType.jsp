...
<body>
 <div class="position">　<a style="font-size:14px;text-decoration:none;color:#FFAA33;" >当前位置：中心概况管理　&gt;&gt;　类型管理　</a>

<div class="div1">
<form action="testCenterTypeServlet?op=add" method="post" name="form1" onsubmit="return formCheck33();">
	...
	<tbody>
            ...
            <a href="testCenterTypeServlet?op=list">刷新列表</a>
            </div>
		</table>
	</tbody>
...
<div class="div2">
<form name="myform" method="post" action="testCenterTypeServlet?op=batchDelete">
	<tbody>
	<table class="right-table" width="50%"  border="0px" cellpadding="0" align="center" cellspacing="0">
		...
		<c:forEach items="${requestScope.testCenterTypeList }"  var="testCenterType" varStatus="status">
		  	<tr style="background-color: ${status.index%2==0?'':'#4F4F4F' } ">
				...
				<td>${testCenterType.typeName }</td>
				<c:if  test="${testCenterType.typeName != '中心概况介绍' && testCenterType.typeName != '欢迎' }" >	
				<td><a href="testCenterTypeServlet?op=delete&id=${testCenterType.id }">删除</a></td>
				</c:if>
			</tr>
		</c:forEach>
	</table>
