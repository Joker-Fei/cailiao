...
<!-- /place -->
	<div class="viewbox">
		<div class="title">
		  <c:forEach items="${requestScope.serviceGuideDetail}"  var="serviceGuideDetail">  				
		<h2><a style="color:#227700;text-decoration:none;">${serviceGuideDetail.title}</a>
		</c:forEach>
		</h2></div>
<!-- /title -->
  <div class="info"> 
  		  <c:forEach items="${requestScope.serviceGuideDetail}"  var="serviceGuideDetail" >  				
  <small>发布时间${serviceGuideDetail.publishTime }</small><!----><small>来源:</small>材料科学与工程学院<small>作者:</small><!-- --> ${serviceGuideDetail.author }<script src="introduce_files/count.htm" type="text/javascript" language="javascript"></script>
  </c:forEach>
  </div>
  <!-- /info -->
<!--  -->
