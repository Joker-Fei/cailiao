...
<body>
 <div class="position">　<a style="font-size:14px;text-decoration:none;color:#FFAA33;" >当前位置：学科介绍　&gt;&gt;　添加学科　</a>

<div class="div1">
<form action="subjecttypeservlet?op=add" method="post" name="form1" onsubmit="return formCheck33();">

	<tbody>

            <div class="search" align="center">
            <span>学科名称：<input maxlength="15" onFocus="if(value==defaultValue){value='';this.style.color='#000'}" onBlur="if(!value){value=defaultValue;this.style.color='#999'}" style="color:#999999;width:300px;height:25px" type="text" name="typeName"/><b>请控制在15个字以内</b></span>
        
            <span><input type="submit" id="search-btn" value="添加"/> </span>
            <a href="subjecttypeservlet?op=list">刷新列表</a>
            </div>
		</table>
	</tbody>
...
<div class="div2">
<form name="myform" method="post" action="subjecttypeservlet?op=batchDelete">
	<tbody>
	<table class="right-table" width="50%"  border="0px" cellpadding="0" align="center" cellspacing="0">
		...
		<c:forEach items="${requestScope.subjectTypeList }"  var="subjectType" varStatus="status">
		  	<tr style="background-color: ${status.index%2==0?'':'#4F4F4F' } ">
				<td> 
					${subjectType.id }
				</td>
				<td>${subjectType.typeName }</td>	
				<td><a href="subjecttypeservlet?op=delete&id=${subjectType.id }">删除</a></td>
			</tr>
		</c:forEach>
	</table>
	</tbody>
