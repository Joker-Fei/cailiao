...
<!-- /place -->
	...
	<div class="viewbox"  style="visibility: none;">
		<div class="title">
		<c:forEach items="${requestScope.instrumentDetail}"  var="instrumentDetail" >  
		
		<h2><a style="color:#227700;text-decoration:none;">${instrumentDetail.typeName}</a>
		</c:forEach>
		</h2></div>
<!-- /title -->
  <div class="info"> 
  <c:forEach items="${requestScope.instrumentDetail}"  var="instrumentDetail" >    
  <small>发布时间${instrumentDetail.publishTime }</small><!----><small>来源:</small>材料科学与工程学院<small>作者:</small><!-- --> ${instrumentDetail.author }<script src="introduce_files/count.htm" type="text/javascript" language="javascript"></script>
  </c:forEach>
  </div>
		...
		<table width="100%">
			...
			<style type="text/css">
					p{text-indent: 2em;}</style>
					 <c:forEach items="${requestScope.instrumentDetail}"  var="instrumentDetail" > 
					<h3>
					<span lang="EN-US">1.</span>仪器介绍  </h3> 
					    
					
					<p class="MsoNormal" >
					${instrumentDetail.content}
					</p>
					</c:forEach>
							</td>
							</tr>
					...
					<c:forEach items="${requestScope.instrumentDetail}"
						var="instrumentDetail">
					<h3>
						<span lang="EN-US">2.</span>生产厂家
					</h3>
					

						<p class="MsoNormal">${instrumentDetail.factory}</p>
					</c:forEach>
					<p class="MsoNormal">
						<span lang="EN-US"><o:p>&nbsp;</o:p></span>
					</p>
					...
					<c:forEach items="${requestScope.instrumentDetail}"
						var="instrumentDetail">
					<h3>
						<span lang="EN-US">3.</span> 主要附件
					</h3>
					

						<p class="MsoNormal">${instrumentDetail.mainAccessories}</p>
					</c:forEach>
					<p class="MsoNormal">
						<span lang="EN-US"><o:p>&nbsp;</o:p></span>
					</p>
					...
					<c:forEach items="${requestScope.instrumentDetail}"
						var="instrumentDetail">
					<h3>
						<span lang="EN-US">4.</span>技术参数   
					</h3>
					

						<p class="MsoNormal">${instrumentDetail.tecParame}</p>
					</c:forEach>
					<p class="MsoNormal">
						<span lang="EN-US"><o:p>&nbsp;</o:p></span>
					</p>
					...
					<c:forEach items="${requestScope.instrumentDetail}"
						var="instrumentDetail">
					<h3>
						<span lang="EN-US">5.</span>技术特点
					</h3>
					

						<p class="MsoNormal">${instrumentDetail.tecFeatures}</p>
					</c:forEach>
					<p class="MsoNormal">
						<span lang="EN-US"><o:p>&nbsp;</o:p></span>
					</p>
					...
					<c:forEach items="${requestScope.instrumentDetail}"
						var="instrumentDetail">
					<h3>
						<span lang="EN-US">6.</span> 应用范围
					</h3>
					

						<p class="MsoNormal">${instrumentDetail.appliRange}</p>
					</c:forEach>
					<p class="MsoNormal">
						<span lang="EN-US"><o:p>&nbsp;</o:p></span>
					</p>
