...
<body>
 <div class="position">　<a style="font-size:14px;text-decoration:none;color:#FFAA33;" >当前位置：计量认证管理　&gt;&gt;　类型管理　</a>

<div class="div1">
<form action="accreditationTypeServlet?op=add" method="post" name="form1" onsubmit="return formCheck33();">
	...
	<tbody>
            ...
            <a href="accreditationTypeServlet?op=list">刷新列表</a>
            </div>
		</table>
	</tbody>
</form>
...
<div class="div2">
<form name="myform" method="post" action="accreditationTypeServlet?op=batchDelete">
	<tbody>
	<table class="right-table" width="50%"  border="0px" cellpadding="0" align="center" cellspacing="0">
		...
		<c:forEach items="${requestScope.accreditationTypeList }"  var="accreditationType" varStatus="status">
		  	<tr style="background-color: ${status.index%2==0?'':'#4F4F4F' } ">
				<td>
					${accreditationType.id }
				</td>
				<td>${accreditationType.typeName }</td>	
			<c:if  test="${accreditationType.typeName != '计量认证介绍'}" >	
				<td><a href="accreditationTypeServlet?op=delete&id=${accreditationType.id }">删除</a></td>
			</c:if>
			</tr>
		</c:forEach>
	</table>
