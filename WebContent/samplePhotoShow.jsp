...
<div class="pright">
	<div class="place"><strong>当前位置:</strong>  <a href="./toTestCenterWeb.jsp">主页</a> &gt; <a>示例照片</a></div>
<!-- /place -->
	<div class="viewbox">
		<div class="title">
		<c:forEach items="${requestScope.samplePhotoDetail}"  var="samplePhotoDetail" >  
		<h2><a style="color:#227700;text-decoration:none;">${samplePhotoDetail.title}</a></h2>
		</c:forEach>
		</div>
<!-- /title -->
  <div class="info"> 
  <c:forEach items="${requestScope.samplePhotoDetail}"  var="samplePhotoDetail" >  
 <small>发布时间${samplePhotoDetail.publishTime }</small><!----><small>来源:</small>材料科学与工程学院<small>作者:</small><!-- -->  ${samplePhotoDetail.author } <script src="introduce_files/count.htm" type="text/javascript" language="javascript"></script>
  </c:forEach>
  </div>
  <!-- /info -->
<!--  -->
...
<!--纵向导航-->
  <div>
   ...
   <c:forEach items="${requestScope.photoTypeList }" var="nn">                             	
			<li>	
				<a href="samplePhotoServlet?op=toSamplePhotoType&typeId=${nn.id }"> ${nn.typeName} </a>
        	 </li>
     </c:forEach> 
