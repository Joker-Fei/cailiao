...
<body>
 <div class="position">　<a style="font-size:14px;text-decoration:none;color:#FFAA33;" >当前位置：科研与合作　&gt;&gt;　类型管理　</a>

<div class="div1">
<form action="coorperationtypeservlet?op=add" method="post" name="form1" onsubmit="return formCheck33();">
	...
	<tbody>
            ...
            <a href="coorperationtypeservlet?op=list">刷新列表</a>
            </div>
		</table>
	</tbody>
...
<div class="div2">
<form name="myform" method="post" action="coorperationtypeservlet?op=batchDelete">
	<tbody>
	<table class="right-table" width="50%"  border="0px" cellpadding="0" align="center" cellspacing="0">
		...
		<c:forEach items="${requestScope.coorperationTypeList }"  var="newsType" varStatus="status">
		  	<tr style="background-color: ${status.index%2==0?'':'#4F4F4F' } ">
				...
				<c:if test="${ newsType.id ge 4}"><!-- 如果ID大于4才显示删除操作 -->
				<td><a href="coorperationtypeservlet?op=delete&id=${newsType.id }">删除</a></td>
				</c:if>
			</tr>
		</c:forEach>
	</table>
